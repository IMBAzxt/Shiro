...
(function(){
    ...
    angular.module('ngTable').factory('NgTableParams', ['$q', '$log', '$filter', 'ngTableDefaults', 'ngTableGetDataBcShim', 'ngTableDefaultGetData', 'ngTableEventsChannel', function($q, $log, $filter, ngTableDefaults, ngTableGetDataBcShim, ngTableDefaultGetData, ngTableEventsChannel) {
        ...
        var NgTableParams = function(baseParameters, baseSettings) {
             ...
             */
            this.url = function(pairs, asString) {
...
//                                    sort["id"]="desc" 改进为 sort:[{name:"id", value:"desc"}]
                                    collectValue(item[subkey], subkey, enckey);
                                }
                            }
                        } else if (!angular.isFunction(item) && isSignificantValue(item, key)) {
                            collectValue(item, enckey);
                        }
                    }
                }
                return pairs;

                function collectValue(value, key, compositeKey){
                    if (asString) {
                        pairs.push(key + "=" + encodeURIComponent(value));
                    } else if(compositeKey != undefined){
                        if( compositeKey == "sorting" ){
                            compositeKey = "sorts";
                        }else if( compositeKey == "filter" ){
                            compositeKey = "wheres";
                        }
                        //TODO groups/havings
                        var subpairs = pairs[compositeKey];
                        if( subpairs == undefined ){
                            subpairs = [];
                        }
                        subpairs.push({"name":key, "value":value});
                        pairs[compositeKey] = subpairs;
                    } else{
            ...
            var settings = {
                ...
                counts: [], // 取消缺省的分页数量选择 [10, 25, 50, 100]
                interceptors: [],
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
	...
	$templateCache.put('ng-table/pager.html', '<div class="ng-cloak ng-table-pager mgNgTabbleDiv" ng-if="params.data.length >= 0"> <div ng-if="params.settings().counts.length > 0" class="ng-table-counts btn-group pull-right"> <button ng-repeat="count in params.settings().counts" type="button" ng-class="{\'active\':params.count()==count}" ng-click="params.count(count)" class="btn btn-default"> <span ng-bind="count"></span> </button> </div> <ul ng-if="pages.length" class="pagination ng-table-pagination"> <li ng-class="{\'myDisabled\': !page.active && !page.current, \'active\': page.current}" ng-repeat="page in pages" ng-switch="page.type"> <a ng-switch-when="prev" ng-click="params.page(page.number)" href="" ng-style="{color:(!page.active && !page.current ? \'#cccccc\':\'\')}">上一页</a> <a ng-switch-when="first" ng-click="params.page(page.number)" ng-style="{\'background-color\':(page.current ? \'#0076de\':\'\')}" href=""><span ng-bind="page.number"></span></a> <a ng-switch-when="more" ng-click="params.page(page.number)" href="">&#8230;</a> <a ng-switch-when="last" ng-click="params.page(page.number)" href="" ng-style="{\'background-color\':(page.current ? \'#0076de\':\'\')}"><span ng-bind="page.number"></span></a> <a ng-switch-when="page" ng-click="params.page(page.number)" href="" ng-style="{\'background-color\':(page.current ? \'#0076de\':\'\')}"><span ng-bind="page.number"></span></a> <a ng-switch-when="next" ng-click="params.page(page.number)" href="" ng-style="{color:(!page.active && !page.current ? \'#cccccc\':\'\')}">下一页</a> </li> </ul> </div> ');
    $templateCache.put('ng-table/sorterRow.html', '<tr class="ng-table-sort-header ngTableHead"> <th title="{{$column.headerTitle(this)}}" ng-repeat="$column in $columns" ng-class="{ \'sortable\': $column.sortable(this), \'sort-asc\': params.sorting()[$column.sortable(this)]==\'asc\', \'sort-desc\': params.sorting()[$column.sortable(this)]==\'desc\' }" ng-click="sortBy($column, $event)" ng-if="$column.show(this)" ng-init="template=$column.headerTemplateURL(this)" class="header {{$column.class(this)}}"> <div ng-if="!template" class="ng-table-header" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'div\'}"><span ng-bind="$column.title(this)" ng-class="{\'sort-indicator\': params.settings().sortingIndicator==\'span\'}"></span></div><div ng-if="template" ng-include="template"></div> </th> </tr> ');
}]);
