...
Enterprise
Blog
 Watch 12  Star 82  Fork 36 sentsin/laydate
 branch: master  laydate/laydate.dev.js
Fetching contributors…

872 lines (783 sloc)  26.605 kb RawBlameHistory
...
Dates.getPath = (function(){
    ...
    for(var i = js.length-1;i>-1;i--){
        if(js[i].src.indexOf("laydate.js")>0){
            jsPath = js[i].src;
            break;
        }
    }
...
Dates.events = function(){
    ...
    Dates.each(S('#laydate_table td'), function(i, elem){
        Dates.on(elem, 'click', function(ev){
            if(!Dates.hasClass(this, as[1])){
                Dates.stopmp(ev);
                Dates.removeClass($('#laydate_table td[class="laydate_click"]')[0],'laydate_click');
                //Dates.removeClass(this,as[1]);
                $(this).removeClass();
                //Dates.removeCssAttr(this,elem.attr("class"))
                Dates.addClass(this, 'laydate_click')
                Dates.creation([this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0]);
            }
            Dates.shde(Dates.query('#'+ as[0]), 0);
            Dates.close();
            //Dates.elem = null;
        });
    });
